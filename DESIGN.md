...
**The No-Colored-Text Rule.** Text is ink (or its opacity steps) on light, white (or white/opacity) on dark. Blue, orange and the highlighter cast are paint — fills for buttons, tags, canvases, focus rings, the logo — **never applied to text** for emphasis or as a link color. Emphasis is weight and size; a link is weight + underline or an arrow, never a hue. **One exception, the instrument readout:** a number that IS the reading of an instrument — the countdown on `/timer.html`, and nothing else so far — may take `pm-orange`, because there the colour is not emphasis added to prose but the state of the thing being read, matched to the orange wedge it belongs to. It is bound by two conditions: the figure sits on a white plinth, never on the brand field (`#ff5500` on `pm-blue` is 1.95:1, unreadable at any size; on white it is 3.19:1 and clears the large-text threshold), and it is display-size, so the large-text threshold is the one that applies. This does not license an orange stat, heading, label or link anywhere else.
...
- **A card may carry one piece of art, on its right.** Reserved for the things you *use* rather than the things you join: "try this" has to show the thing, where "join this" only has to name it. The art is the REAL component off the page it advertises, never a redrawing — two copies of the same object drift the first time one changes, and a social card is exactly where nobody notices. An art card's headline runs **bigger** than the standard 96px, not smaller (144px on the timer): two words beside an illustration have to hold their own. Headline and art share one horizontal budget, so size them together and measure the longest word at the real font rather than guessing — every 40px handed back to the art costs the headline roughly 8px.
- **The wordmark, the headline and the line under it start on one vertical.** `logo.png` carries 128px of transparent padding either side of its 1264x563 canvas, so dropped in unaltered its visible mark sits inside its own box and lands right of the type. Pull it back by that padding at whatever height it renders (`calc(<height>*-128/563)`) — the same correction the navbar makes.
...
- **The Watermark Rule.** The "Makers" script appears in backgrounds at ONE treatment only, implemented once in `pm/script-watermark.tsx` — never hand-roll a second. The BLACK script, zoomed to 180% of the band's width and centered, so the letterforms bleed off every edge and the word never reads in full. Opacity is a whisper that darkens the ground tone-on-tone: 8% on blue set-pieces (reads as a deeper blue), 5% on light grounds (reads as a soft grey). No blur, no rotation beyond the script's natural slant, always `aria-hidden` and behind every content layer. **In a band taller than the script's own height at 180%, the width goes up until it bleeds again** — same asset, same opacity, same centring, only the number gives way. The 180% figure assumes a band roughly as shallow as a quote carousel; once a band also carries its headline and body copy (the two audience bands on /mentoring), a 180% script stops short of the top and bottom and reads as a stamp floating in the middle rather than a ground. Raise it until the letterforms run off both edges: the rule's requirement is the bleed, not the percentage. Pass the width per band rather than changing the component. **One exception, the navbar's mega-menu panel:** there the script is sized to the panel's WIDTH so it runs edge to edge and the word reads in full, bleeding off the top and bottom only. A band that wide and shallow cut the first and last letters vertically down their stems, which reads as a mistake rather than a bleed. Same asset, dropped to **3%**: at this scale the letterforms cover most of the panel, and 5% stopped being a whisper. It exists to fill the empty bottom of an open menu. The ground it sits on is always the brand wash: blue set-pieces run the 135° `blue-bright → blue → blue-press` gradient (plus at most a soft glow orb); light grounds stay paper with a gentle `blue-soft`/lavender wash.
