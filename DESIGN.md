...
- **Master hero** — a blue *drenched* field: `pm-blue` gradient, floating orbs (`pm-float`), a blurred script-SVG "Makers" watermark, and a workspace screenshot in a browser frame. The loudest surface in the system.
...
- **Motion primitives:** `Reveal` (fade + directional slide, optional blur-in, `700ms` `cubic-bezier(0.2,0,0,1)`), `Stagger` (cascades reveals ~90ms apart), `CountUp` (stats animate 0→value on scroll, `tabular-nums`), `Marquee` (seamless revolving strip, pause-on-hover, edge-fade), `Magnetic` (a CTA leans toward the cursor). Entrance uses `.pm-rise`. **Every one is reduced-motion safe** — content is forced visible and numbers jump to final. Ease-out only, no bounce; layout properties are never animated.
